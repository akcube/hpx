...
namespace hpx { namespace parallel { namespace util { namespace detail {
    ...
    using is_arithmetic_multiplies_reduction = std::integral_constant<bool,
        std::is_arithmetic_v<T> and std::is_same_v<Operation, std::multiplies<T>>>;
    ...
    using is_arithmetic_logical_and_reduction = std::integral_constant<bool,
        std::is_arithmetic_v<T> and std::is_same_v<Operation, std::logical_and<T>>>;

    template <typename T, typename Operation>
    using is_arithmetic_logical_or_reduction = std::integral_constant<bool,
        std::is_arithmetic_v<T> and std::is_same_v<Operation, std::logical_or<T>>>;
    ...
    struct reduce_n {
        ...
        template<typename Iter1, typename T, typename Convert,
            typename Reduce>
        HPX_HOST_DEVICE HPX_FORCEINLINE static typename std::enable_if<
            is_arithmetic_plus_reduction<T, Reduce>::value,
            T>::type reduce(Iter1 HPX_RESTRICT it, std::size_t count, T init, Reduce /* */,
            Convert conv){
        ...
        template<typename Iter1, typename T, typename Convert,
            typename Reduce>
        HPX_HOST_DEVICE HPX_FORCEINLINE static typename std::enable_if<
            is_arithmetic_minus_reduction<T, Reduce>::value,
            T>::type reduce(Iter1 HPX_RESTRICT it, std::size_t count, T init, Reduce /* */,
            Convert conv){
            HPX_VECTOR_REDUCTION(- : init)
            for(std::size_t i = 0; i < count; i++){
        ...
        template<typename Iter1, typename T, typename Convert,
            typename Reduce>
        HPX_HOST_DEVICE HPX_FORCEINLINE static typename std::enable_if<
            is_arithmetic_multiplies_reduction<T, Reduce>::value,
            T>::type reduce(Iter1 HPX_RESTRICT it, std::size_t count, T init, Reduce /* */,
            Convert conv){
            HPX_VECTOR_REDUCTION(* : init)
            for(std::size_t i = 0; i < count; i++){
        ...
        template<typename Iter1, typename T, typename Convert,
            typename Reduce>
        HPX_HOST_DEVICE HPX_FORCEINLINE static typename std::enable_if<
            is_arithmetic_bit_and_reduction<T, Reduce>::value,
            T>::type reduce(Iter1 HPX_RESTRICT it, std::size_t count, T init, Reduce /* */,
            Convert conv){
            HPX_VECTOR_REDUCTION(& : init)
            for(std::size_t i = 0; i < count; i++){
        ...
        template<typename Iter1, typename T, typename Convert,
            typename Reduce>
        HPX_HOST_DEVICE HPX_FORCEINLINE static typename std::enable_if<
            is_arithmetic_bit_or_reduction<T, Reduce>::value,
            T>::type reduce(Iter1 HPX_RESTRICT it, std::size_t count, T init, Reduce /* */,
            Convert conv){
            HPX_VECTOR_REDUCTION(| : init)
            for(std::size_t i = 0; i < count; i++){
        ...
        template<typename Iter1, typename T, typename Convert,
            typename Reduce>
        HPX_HOST_DEVICE HPX_FORCEINLINE static typename std::enable_if<
            is_arithmetic_bit_xor_reduction<T, Reduce>::value,
            T>::type reduce(Iter1 HPX_RESTRICT it, std::size_t count, T init, Reduce /* */,
            Convert conv){
            HPX_VECTOR_REDUCTION(^ : init)
            for(std::size_t i = 0; i < count; i++){
        ...
        template<typename Iter1, typename T, typename Convert,
            typename Reduce>
        HPX_HOST_DEVICE HPX_FORCEINLINE static typename std::enable_if<
            is_arithmetic_logical_and_reduction<T, Reduce>::value,
            T>::type reduce(Iter1 HPX_RESTRICT it, std::size_t count, T init, Reduce /* */,
            Convert conv){
            HPX_VECTOR_REDUCTION(&& : init)
            for(std::size_t i = 0; i < count; i++){
        ...
        template<typename Iter1, typename T, typename Convert,
            typename Reduce>
        HPX_HOST_DEVICE HPX_FORCEINLINE static typename std::enable_if<
            is_arithmetic_logical_or_reduction<T, Reduce>::value,
            T>::type reduce(Iter1 HPX_RESTRICT it, std::size_t count, T init, Reduce /* */,
            Convert conv){
            HPX_VECTOR_REDUCTION(|| : init)
            for(std::size_t i = 0; i < count; i++){
        ...
        template<typename Iter1, typename T, typename Convert,
            typename Reduce>
        HPX_HOST_DEVICE HPX_FORCEINLINE static typename std::enable_if<
            is_not_omp_reduction<T, Reduce>::value, T>::type
        reduce(Iter1 HPX_RESTRICT it, std::size_t count, T init, Reduce r, Convert conv){
            ...
            if(count <= (block_size << 1)){
                ...
                for(std::size_t i = 0; i < (block_size << 1); i+=2){
                    ::new (tblock + i/2) T(HPX_INVOKE(r, HPX_INVOKE(conv, *it), HPX_INVOKE(conv, *(it + 1))));
                    it += 2;
                }
                /* Vectorized loop */
                const std::size_t limit = block_size * (count / block_size);
                for(std::size_t i = (block_size << 1); i < limit; i += block_size){
                    HPX_VECTORIZE
                    for(std::size_t j = 0; j < block_size; j++){
                        tblock[j] = HPX_INVOKE(r, tblock[j], HPX_INVOKE(conv, *(it + j)));
                    }
                    it += block_size;
                }
                /* Remainder */
    ...
    ///////////////////////////////////////////////////////////////////////////
    struct binary_reduce_n {
        /* Will only be called when the iterators are all random access */
        template<typename Iter1, typename Iter2, typename T, typename Convert,
            typename Reduce>
        HPX_HOST_DEVICE HPX_FORCEINLINE static typename std::enable_if<
            is_arithmetic_plus_reduction<T, Reduce>::value,
            T>::type reduce(Iter1 HPX_RESTRICT it1, Iter2 HPX_RESTRICT it2, std::size_t count, T init,
            Reduce /* */, Convert conv){
            HPX_VECTOR_REDUCTION(+ : init)
            for(std::size_t i = 0; i < count; i++){
        ...
        HPX_HOST_DEVICE HPX_FORCEINLINE static typename std::enable_if<
            is_not_omp_reduction<T, Reduce>::value, T>::type
        reduce(Iter1 HPX_RESTRICT it1, Iter2 HPX_RESTRICT it2, std::size_t count, T init, Reduce r,
            Convert conv){
            const std::size_t block_size = HPX_LANE_SIZE / sizeof(T);
            ...
            if(count <= (block_size << 1)){
                ...
                /* Initialize block[i] = r(f(2*i), f(2*i + 1)) */
                for(std::size_t i = 0; i < (block_size << 1); i+=2){
                    ::new (tblock + i) T(HPX_INVOKE(r,
                        HPX_INVOKE(conv, *it1, *it2),
                ...
                HPX_VECTORIZE
                for(std::size_t i = 0; i < count - limit; i++){
                    tblock[i] = HPX_INVOKE(r, tblock[i], HPX_INVOKE(conv, *it1, *it2));
                    ++it1, ++it2;
                }
                /* Merge */
                for(std::size_t i = 0; i < block_size; i++){
